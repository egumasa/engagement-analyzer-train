...
from spancat_eval_confusion import evaluate_spancat
...
def evaluate(
    ...
    corpus = Corpus(data_path,
                    gold_preproc=gold_preproc)  #https://spacy.io/api/corpus
    ...
    for metric, key in metrics.items():
        if key in scores:
            if key == "cats_score":
                metric = metric + " (" + scores.get("cats_score_desc",
                                                    "unk") + ")"
    ...
    data = handle_scores_per_type(scores,
                                  data,
                                  spans_key=spans_key,
                                  silent=silent)
    ...
    *,
    ...
    if f"spans_{spans_key}_per_type" in scores:
        if scores[f"spans_{spans_key}_per_type"]:
            print_prf_per_type(msg, scores[f"spans_{spans_key}_per_type"],
                               "SPANS", "type")
            data[f"spans_{spans_key}_per_type"] = scores[
                f"spans_{spans_key}_per_type"]
    if "cats_f_per_type" in scores:
        if scores["cats_f_per_type"]:
            print_prf_per_type(msg, scores["cats_f_per_type"], "Textcat F",
                               "label")
