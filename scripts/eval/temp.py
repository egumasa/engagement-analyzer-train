...
DEFAULT_PIPELINE = ("senter", "tagger", "morphologizer", "parser", "ner",
                    "textcat")
...
class PRFScore:
    ...
    def __add__(self, other):
        return PRFScore(tp=self.tp + other.tp,
                        fp=self.fp + other.fp,
                        fn=self.fn + other.fn)
...
class Scorer:
    ...
    def score(self, examples: Iterable[Example]) -> Dict[str, Any]:
        ...
        if hasattr(self.nlp.tokenizer, "score"):
            scores.update(self.nlp.tokenizer.score(examples,
                                                   **self.cfg))  # type: ignore
    ...
    @staticmethod
    def score_tokenization(examples: Iterable[Example],
                           **cfg) -> Dict[str, Any]:
    ...
    def score_spans(
        ...
        **cfg,
        ...
        for example in examples:
            ...
            if has_annotation is None or (has_annotation is not None
                                          and has_annotation(pred_doc)):
        ...
        **cfg,
        ...
        for example in examples:
            ...
            if multi_label:
                ...
                pred_label, pred_score = max(pred_cats.items(),
                                             key=lambda it: it[1])
                gold_label, gold_score = max(gold_cats.items(),
                                             key=lambda it: it[1])
        ...
        macro_auc = (sum(auc.score if auc.is_binary() else 0.0
                         for auc in auc_per_type.values()) / n_cats)
        results: Dict[str, Any] = {
            ...
            f"{attr}_f_per_type":
            {k: v.to_dict()
             for k, v in f_per_type.items()},
            f"{attr}_auc_per_type": {
                k: v.score if v.is_binary() else None
                for k, v in auc_per_type.items()
            },
        }
        if len(labels) == 2 and not multi_label and positive_label:
            positive_label_f = results[f"{attr}_f_per_type"][positive_label][
                "f"]
    ...
    @staticmethod
    def score_links(examples: Iterable[Example], *,
                    negative_labels: Iterable[str], **cfg) -> Dict[str, Any]:
        ...
        for example in examples:
            ...
            for gold_ent in example.reference.ents:
                gold_ent_by_offset[(gold_ent.start_char,
                                    gold_ent.end_char)] = gold_ent
    ...
    def score_deps(
        ...
        **cfg,
        ...
        for example in examples:
                ...
                if gold_i not in missing_indices:
                    ...
                    if dep not in ignore_labels and token.orth_.strip():
                        ...
                        if gold_i is None or gold_head is None:
                            ...
                            pred_deps_per_dep[dep].add(
                                (gold_i, gold_head, dep))
            ...
            unlabelled.score_set(set(item[:2] for item in pred_deps),
        ...
        if len(unlabelled) > 0:
            return {
                ...
                {k: v.to_dict()
                 for k, v in labelled_per_dep.items()},
            }
        else:
            return {
                f"{attr}_uas": None,
